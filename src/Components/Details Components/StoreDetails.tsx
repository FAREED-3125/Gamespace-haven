...
const StoreDetails = ({ details, setOpenDetails }: DevDetailsProps) => {
  const {
    data: gameData,
    err: gameErr,
    loading: gameLoading,
          ...
          {gameLoading ? (
            ...
            <>
              ...
              <div className="px-3 lg:h-full lg:w-full overflow-auto scroll-d">
                ...
                <a
                  className="flex gap-1 text-gray-400 underline items-center mb-2"
                  href={details?.domain || gameData?.domain}
                >
                  {details?.domain ||
                    gameData?.domain ||
                ...
                <div className="px-3 py-2 bg-darkBlue font-[600] rounded-md w-max mb-3">
                  Game count: {gameData?.games_count || details?.games_count}
                </div>
